...
// RegisterGRPC registers the Sophia service on a gRPC server.
func RegisterGRPC(s grpc.ServiceRegistrar) {
...
func (s *Server) CreateIdentity(ctx context.Context, req *pb.CreateIdentityRequest) (*pb.CreateIdentityResponse, error) {
	...
	id.Description = fmt.Sprintf("%s %s\n\nMotto: %s", id.GivenName, id.FamilyName, id.Motto)
	...
	if err := identity.WriteHolonYAML(id, outputPath); err != nil {
		return nil, status.Errorf(codes.Internal, "write holon.yaml: %v", err)
	}
...
func defaultOutputDir(id identity.Identity) string {
	dirName := strings.ToLower(id.GivenName + "-" + strings.TrimSuffix(id.FamilyName, "?"))
	dirName = strings.ReplaceAll(dirName, " ", "-")
	return filepath.Join("holons", dirName)
}
