...
func TestContractPinVersionNominal(t *testing.T) {
	root := t.TempDir()
	client, cleanup := startContractMemClient(t, root)
	defer cleanup()

	created, err := client.CreateIdentity(context.Background(), validCreateReq(filepath.Join("holons", "pin-nominal")))
	if err != nil {
		t.Fatalf("CreateIdentity failed: %v", err)
	}

	pinResp, err := client.PinVersion(context.Background(), &pb.PinVersionRequest{
		Uuid:          created.GetIdentity().GetUuid(),
		BinaryPath:    "/usr/local/bin/sophia",
		BinaryVersion: "1.2.3",
		Os:            "linux",
		Arch:          "amd64",
	})
	...
	if pinResp.GetIdentity().GetBinaryVersion() != "1.2.3" {
		t.Fatalf("binary_version = %q, want %q", pinResp.GetIdentity().GetBinaryVersion(), "1.2.3")
	}
	if pinResp.GetIdentity().GetOs() != "linux" {
		t.Fatalf("os = %q, want %q", pinResp.GetIdentity().GetOs(), "linux")
	}
	if pinResp.GetIdentity().GetArch() != "amd64" {
		t.Fatalf("arch = %q, want %q", pinResp.GetIdentity().GetArch(), "amd64")
	}

	showResp, err := client.ShowIdentity(context.Background(), &pb.ShowIdentityRequest{Uuid: created.GetIdentity().GetUuid()})
	if err != nil {
		t.Fatalf("ShowIdentity after pin failed: %v", err)
	}
	if showResp.GetIdentity().GetBinaryVersion() != "1.2.3" {
		t.Fatalf("persisted binary_version = %q, want %q", showResp.GetIdentity().GetBinaryVersion(), "1.2.3")
	}
}

func TestContractPinVersionNotFound(t *testing.T) {
	root := t.TempDir()
	client, cleanup := startContractMemClient(t, root)
	defer cleanup()

	_, err := client.PinVersion(context.Background(), &pb.PinVersionRequest{Uuid: "missing-uuid"})
	if err == nil {
		t.Fatal("expected not found error")
	}
	if got := status.Code(err); got != codes.NotFound {
